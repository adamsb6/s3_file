...
s3_file can decrypt files that have been encrypted using an AES-256-CBC cipher.  The encryption utilities in `bin` require the gem package `trollop`.  You do not need trollop to use the s3_file resource.

To use s3_file with encrypted files:

1. Create a new key using `bin/new_key > my_new_key`.
1. Encrypt your file using the new key by calling `bin/encode_file -k my_new_key -s my_source_file [ -d my_destination_file ]`.
1. You can test decryption of your file using `bin/decode_file -k my_new_key -s my_encoded_file [ -d my_decoded_destionation ]`.
1. Upload your encrypted file to S3 as normal.
1. In the s3_file resource call, include the string in `my_new_key` for the resource to decrypt the file.
...
To create your cipher, run `bin/new_key` and a new 256-bit (32 hexidecimal characters) will be generated for you. Paste that key into a file for later use.  DO NOT include an endline in the file otherwise the encryption and decryption will fail.

Try `bin/new_key > my_new_key`.

You can use the utility `bin/encode_file` to encrypt files prior to uploading to S3.

You can use the utility `bin/decrypt_file` to decrypt files prior to make sure the encryption is working.
